...
internal static unsafe class Native
{
    ...
    internal const int BR_GID       = 1;        // base gid (we add workerIndex)
    ...
    // ---- io_uring basic opaque structs ----
    [StructLayout(LayoutKind.Sequential)]
    internal struct io_uring { internal fixed ulong _[128]; } // opaque placeholder

    [StructLayout(LayoutKind.Sequential)]
    internal struct io_uring_sqe { internal fixed ulong _[8]; }
    ...
    [DllImport("libc")] internal static extern int sched_getcpu();
    ...
    internal static class Affinity
    {
        ...
        public static int GetCurrentCpu() => sched_getcpu();
    ...
    // ---- Optional: NUMA helpers (libnuma). Safe fallbacks provided. ----
    internal static class Numa
    {
        // libnuma APIs (if present on system)
        [DllImport("libnuma.so.1", EntryPoint = "numa_available", CallingConvention = CallingConvention.Cdecl)]
        private static extern int _numa_available();
    
        [DllImport("libnuma.so.1", EntryPoint = "numa_max_node", CallingConvention = CallingConvention.Cdecl)]
        private static extern int _numa_max_node();
    
        [DllImport("libnuma.so.1", EntryPoint = "numa_node_of_cpu", CallingConvention = CallingConvention.Cdecl)]
        private static extern int _numa_node_of_cpu(int cpu);
    
        [DllImport("libnuma.so.1", EntryPoint = "numa_alloc_onnode", CallingConvention = CallingConvention.Cdecl)]
        private static extern void* _numa_alloc_onnode(nuint size, int node);
    
        [DllImport("libnuma.so.1", EntryPoint = "numa_free", CallingConvention = CallingConvention.Cdecl)]
        private static extern void _numa_free(void* ptr, nuint size);

        public static bool IsAvailable()
        {
            try
            {
                return _numa_available() >= 0;
            }
            catch(Exception e)
            {
        ...
        public static int MaxNodeHint()
        {
            try { return _numa_max_node() + 1; } catch { return 1; }
        }

        public static int NodeOfCpu(int cpu)
        {
            try { return _numa_node_of_cpu(cpu); } catch { return -1; }
        }

        public static byte* AllocOnNode(nuint size, int node)
        {
            try { return (byte*)_numa_alloc_onnode(size, node); } catch { return null; }
        }

        public static void Free(void* ptr, nuint size)
        {
            try { _numa_free(ptr, size); } catch { }
        }
    }
}
